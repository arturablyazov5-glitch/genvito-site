(async () => {
  ...
  function render(state, data = {}) {
    ...
    if (state === 'success') {
      ...
      title.textContent = 'Спасибо! Оплата прошла';
      message.textContent = data.paymentMethodSaved
      ...
      plan.textContent = data.plan || pending.plan || 'Оплаченный тариф';
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    const response = await fetch(`${window.SUPABASE_CONFIG.url}/functions/v1/payment-api/payment-status?${query}`);
    const data = await response.json().catch(() => ({}));
    if (response.ok && data.status === 'succeeded' && data.paid && data.activated) return render('success', data);
