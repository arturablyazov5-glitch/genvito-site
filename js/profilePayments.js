(async () => {
  ...
  list.innerHTML = methods.map((method) => {
    ...
    return `<div class="payment-method-row" data-method-id="${safe(method.id)}" data-primary="${method.isPrimary ? 'true' : 'false'}"><img src="${iconSrc}" alt="${iconAlt}" class="payment-method-card-icon${isYooMoney ? ' is-payment-logo' : ''}${isSbp ? ' is-sbp-logo' : ''}"><div class="payment-method-copy"><strong>${safe(name)}</strong><small>${safe(captionText)}</small><button class="payment-method-primary" type="button" aria-pressed="${method.isPrimary ? 'true' : 'false'}"${available ? '' : ' disabled'}>${primaryLabel}</button></div><button class="payment-method-remove" type="button" aria-label="Отвязать способ оплаты" title="Отвязать способ оплаты"><i data-lucide="trash-2" aria-hidden="true"></i></button></div>`;
  }).join('');
